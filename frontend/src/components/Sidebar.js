...
//     }
    ...
    async function followUser(userToFollow, usernameToFollow) {
        try {
        const {data} = await axios.put(`http://localhost:5000/api/users/${userToFollow}/follow`, {userId},
         {
            headers: {
                Authorization: `Bearer ${userInfo.token}`,
            }
  ...
  : Swal.fire(error.message);
    ...
    async function unfollowUser(userToUnfollow, usernameToUnfollow) {
        try {
       const {data} = await axios.put(`http://localhost:5000/api/users/${userToUnfollow}/unfollow`, {userId},
         {
            headers: {
                Authorization: `Bearer ${userInfo.token}`,
            }
