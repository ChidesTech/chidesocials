...
export default function Feed(props) {
    ...
    async function getMyPosts(userId) {
        try {
            setLoading(true)
                ...
                setLoading(false);
            const { data } = await axios.get(`http://localhost:5000/api/posts/mine/${userId}`,{
                headers: {
    ...
    async function getUserPosts(params) {
        ...
        try {
            ...
            const { data } = await axios.get(`http://localhost:5000/api/posts/mine/${id}`,
            {
                headers: {
                    Authorization: `Bearer ${userInfo.token}`,
                }
    ...
    async function getAllPosts(userId) {
        try {
            ...
            const { data } = await axios.get(`http://localhost:5000/api/posts/all/${userId}`,  {
                headers: {
                    Authorization: `Bearer ${userInfo.token}`,
                }
            });
    ...
    async function getSearchPosts(searchText) {
        ...
        try {
            setLoading(true)
            const { data } = await axios.get(`http://localhost:5000/api/search?searchText=${searchText}`, {
                headers: {
                    Authorization: `Bearer ${userInfo.token}`,
                }
            })
                        ...
                        posts.length === 0 ? <div className="alert p-2 m-1 alert-info">No Post found.</div> :
                            ...
                            posts.map(post => {
                                return <Post key={post._id}
                                    _id={post._id} postUserId={post.user._id} key={post._id} image={post.image} desc={post.desc}
                                    ...
                                    dislikers={post.dislikes} createdAt={post.createdAt}
                                />
                            }))
            }
            {
